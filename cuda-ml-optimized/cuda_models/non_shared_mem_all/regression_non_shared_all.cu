...
// CUDA kernel for forward pass, loss calculation, and gradient calculation using global memory
__global__ void fused_kernel(float* X, float* y_true, float* w, float* b, float* y_pred, float* loss, float* w_grad, float* b_grad, int n) {
    ...
    if (i < n) {
        ...
        // Update global memory atomically to avoid race conditions
        atomicAdd(loss, local_loss);
        atomicAdd(w_grad, local_w_grad);
        atomicAdd(b_grad, local_b_grad);
    }
}
